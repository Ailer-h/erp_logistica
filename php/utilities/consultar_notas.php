<?php
    ...
    $query = mysqli_query($conexao, "select nt.id_nota,nt.produto_nota,nt.qtd_produto,nt.data_nota, es.unidade_medida, es.nome_produto, nt.estado_nota FROM nota nt, estoque es where nt.id_empresa = $id and nt.produto_nota = es.id_materiaPrima and es.nome_produto like '%$search%' $filter;");
    
    while($infos_nota = mysqli_fetch_array($query)){
        
        $date = implode("/", array_reverse(explode("-", $infos_nota[3])));
        
        $cancelado = $infos_nota[6] == "Cancelada" ? "disabled" : "";
        $corButton = $infos_nota[6] == "Cancelada" ? "secondary" : "warning";
